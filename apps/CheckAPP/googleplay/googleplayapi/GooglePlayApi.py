...
from apps.CheckAPP.googleplay.googleplayapi.encrypted import EncryptedKey
import apps.CheckAPP.googleplay.googleplayapi.keystore as PUB_KEY
try:
    # For Python 3
...
import apps.CheckAPP.googleplay.googleplayapi. googleplay_pb2 as googleplay_pb2
...
class GooglePlayAPI(object):
    ...
    def __init__(self, androidId, email, password, google_token=None, lang=None, debug=False, login=True,
        ...
        self._google_signin_cookiejar = Cookiejar.CookieJar()
        self._google_signin_session = build_opener(HTTPCookieProcessor(self._google_signin_cookiejar))
        self._android_auth_cookiejar = Cookiejar.CookieJar()
        self._android_auth_session = build_opener(HTTPCookieProcessor(self._android_auth_cookiejar))
        self.email = email
        self.psw = password
    ...
    def _get_embedded_token(self, txt):
        ...
        dic = json.loads(txt.decode('utf-8')[txt.find(b'{'):txt.find(b'}') + 1])

        # freq = txt[txt.find('en_us&quot;,&quot;') + len('en_us&quot;,&quot;'):]
        freq = txt.decode('utf-8')[txt.find(b'en_us&quot;,&quot;') + len(b'en_us&quot;,&quot;'):]

        freq_token = freq[:freq.find('&')]
        lines = dic['OewCAd'].split('\n')
        if len(lines) > 1:
            freq_token1 = lines[1][2:-2]
        else:
            freq_token1= "Error: Not enough lines in the list"

        return freq_token1, freq_token
    ...
    def toDict(self, protoObj):
        ...
        iterable = False
        if isinstance(protoObj, (RepeatedCompositeFieldContainer, RepeatedCompositeFieldContainer)):
            iterable = True
        else:
            protoObj = [protoObj]
        retlist = []
        ...
        for po in protoObj:
            ...
            for fielddesc, value in po.ListFields():
                ...
                if fielddesc.type == descriptor.FieldDescriptor.TYPE_GROUP or \
                    isinstance(value, (RepeatedCompositeFieldContainer, RepeatedCompositeFieldContainer)) or \
    ...
    def loginV2(self, accountType=ACCOUNT_TYPE_HOSTED_OR_GOOGLE):
        # try:
        encryptedPassword = gpsoauth.google.construct_signature(self.email, self.psw,  gpsoauth.ANDROID_KEY_7_3_29)
        ...
        print(params1)
        # resp = requests.post(self.URL_LOGIN, params1)
        with requests.Session() as session:
              resp=self.do_post(self.URL_LOGIN,session,params1)
    ...
    def do_post(self,login_url,session,params):
        response = session.post(login_url, data=params, headers={"Content-Type": "application/x-www-form-urlencoded"})
        print("你要的结果："+response.text)
        map_result = self.parse_content(response.text)
        if response.status_code == 200:
            return map_result

        if "Error" in map_result:
            raise Exception(map_result["Error"])
    ...
    def _lookup_request(self, azt, freq_token):
        ...
        full_url = 'https://accounts.google.com/_/lookup/accountlookup?' + urlencode(params)
        # resp = self._google_signin_session.open(full_url, data=urlencode(data).encode('utf-8'))

        resp = self.google_signin_session.post('https://accounts.google.com/_/lookup/accountlookup', params=params,
                        headers=headers,  data=urlencode(data).encode('utf-8'))
        print(resp.text)
    ...
    def _get_google_token(self):
        ...
        try:
            ...
            resp = self._android_auth_session.open('https://android.clients.google.com/auth',
                                                   data=urlencode(first_sigin_data))
    ...
    def _get_android_auth_token(self):
        ...
        try:
            resp = self._android_auth_session.open('https://android.clients.google.com/auth',
                                                   data=urlencode(androidmarket_service_post_data))
            resp_text = _get_resp_content(resp)
            resp_dict = {line.split('=')[0]: line.split('=')[1] for line in resp_text.splitlines() if line}
